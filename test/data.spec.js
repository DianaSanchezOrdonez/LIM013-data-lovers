...
describe('calcular la vida por nivel (hp)', () => {
  ...
  it('Ahri:674.400', () => {
    const data = [{ name: 'Aatrox' ,stats: { hp: 537.8, hpperlevel: 85}}, { name: 'Ahri' ,stats: { hp: 514.4, hpperlevel: 80}}, { name: 'Akali' ,stats: { hp: 587.8, hpperlevel: 85}}];
    const position = 1;
    const level = 2;
    const dataResult = 674.400;
    expect(order.nameChampionZa(data,position,level)).toEqual(dataResult);
  });
});
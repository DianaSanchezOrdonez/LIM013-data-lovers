...
describe('filtar por daño_ataque (filterRange)', () => {
  ...
  it('returns `rango2`', () => {
    const filterDaño_Ataque = [{ name: 'Ahri', stats: { attackrange: 231 } }, { name: 'Zyra', stats: { attackrange: 336 } }, { name: 'Diana', stats: { attackrange: 335 } }]
    const range = 'rango2';
    const result = [{ name: 'Ahri', stats: { attackrange: 231 } }, { name: 'Diana', stats: { attackrange: 335 } }];
    expect(order.filterRange(filterDaño_Ataque, range)).toEqual(result);
  })

  it('returns `rango3`', () => {
    const filterDaño_Ataque = [{ name: 'Graves', stats: { attackrange: 336 } }, { name: 'Alistar', stats: { attackrange: 440.5 } }]
    const range = 'rango3';
    const result = [{ name: 'Graves', stats: { attackrange: 336 } }];
    expect(order.filterRange(filterDaño_Ataque, range)).toEqual(result);
  })
